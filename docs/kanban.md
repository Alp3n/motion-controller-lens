...
- [x] **Lampy LR/LG automatyczne wg stanu maszyny, 2026-09-12**
      (zamówienie: „LR ma się włączać adekwatnie do swojej roli" +
      dopełnienie tego samego dnia: „zielona lampa powinna włączyć się,
      jeśli maszyna jest gotowa"). LR świeci sam w stanie ALARM, LG w
      stanie READY, obie gasną poza swoim stanem —
      `_apply_signal_lamps()` w `_io_modbus_poll_loop`, porównanie z
      odczytem (samonaprawiające się, nie z pamięcią). Operator ma
      równolegle własne kroki WYJSCIE na LG w cyklu produkcyjnym
      (gaśnie/zapala na początku/końcu cyklu) — redundantne z automatem,
      nie sprzeczne. **LY bez automatycznej roli** — tylko krok WYJSCIE
      cyklu. „Alarmy..." z pierwszego zgłoszenia wciąż urwane — możliwe
      że operator miał na myśli coś szerszego niż `MachineState.ALARM`
      (np. alarmy zużycia, temat M) — do wyjaśnienia. Szczegóły:
      `zmiany/lampy-sygnalizacyjne-automatyczne.md`.
